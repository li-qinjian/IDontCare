...
# make bin folder.
New-Item -Path "$dist\$Name\" -Name "bin" -ItemType "directory"
New-Item -Path "$dist\$Name\bin" -Name "Win64_Shipping_Client" -ItemType "directory"

# Copy the built assembly in place.
Copy-Item "$Dll" -Destination "$dist\$Name\bin\Win64_Shipping_Client"
Copy-Item "$Pdb" -Destination "$dist\$Name\bin\Win64_Shipping_Client"
